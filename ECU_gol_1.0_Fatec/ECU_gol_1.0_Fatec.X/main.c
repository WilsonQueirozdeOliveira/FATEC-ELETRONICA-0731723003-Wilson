...
void interrupt high_priority interupcoes(void)
{
    if (INTCONbits.TMR0IF == 1) //Esta interrupcao ocorre a cada 0,1 ms
    {
        ...
        //-----------------------------------------------------------------------------
        //entrada analÛgica AN0 - Sinal do ACT
    ...
    if (INT0IE && INT0IF)
    {
        ...
        {   
            //Se a rotaÁao > 450, injeta na borda de subida a cada dois pulsos
            if ((rotacao>450)&&(FF_Borda==1)&&(Conta_PIP>2))        
            {
                Saida_Bico = 1;     //Liga o bico injetor
                T_inj      = 26;        //carrega o tempo de injeÁao
                tinj       = T_inj;     //coloca no buffer do display
                Conta_PIP  = 0;
            }
            else
        ...
        if (FF_Borda == 1)
        {
            f_RPM=t_RPM;            //Atualiza rota√ß√£o no display
...
TRISE = 0XFF;           // Define como entradas (Bot√µes)
...
// Ver tabela da p√°gina 260 do Manual do PIC 18F4550 pra outras configuracoes
